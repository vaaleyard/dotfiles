...
static battery_info_t battery_get_info() {
  ...
  info.energy_now = freadint("/sys/class/power_supply/BAT0/charge_now");
  info.energy_full = freadint("/sys/class/power_supply/BAT0/charge_full");
...
static int fetch_wifi_info(char *wifi_str) {
  int len = 0;
  wifi_status_t wifi = fread_wifi_status("/sys/class/net/wlp6s0/operstate");

  if(wifi == UP) {
    len += snprintf(wifi_str, STRSZ, "  ");
  } else {
    len += snprintf(wifi_str, STRSZ, " no wifi ");
  }
...
static int fetch_current_time(char *time_str) {
  ...
  return strftime(time_str, STRSZ, "  %H:%M ", current_tm);
}
...
int main(void) {
  ...
  for (;;sleep(20)) {
    ...
    snprintf(status, STATUSSZ, "%s %s %s", time, battery, wifi);
